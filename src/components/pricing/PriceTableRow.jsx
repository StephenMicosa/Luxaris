...
export default function PriceTableRow({ item, index = 0, hoverColor = "rose-50" }) {
    const animationDelay = 0.3 + (index * 0.1);

    return (
        <>
            {index === 0 && <style>{rowStyles}</style>}
            <tr className={`hover:bg-${hoverColor} transition-colors price-row`} style={{ animationDelay: `${animationDelay}s` }}>
